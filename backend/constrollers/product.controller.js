...
export const getProducts = async (req, res)=> {
    try {
        const products = await Product.find({}); //esse {} significa que vai receber todos os produtos
        res.status(200).json({ success: true, data: products})
    
    } catch (error) {
        console.log("Erro em ver todos os produtos", error.message);
...
export const createProduct = async (req, res) =>{
    const product = req.body; //usuário vai enviar esse dado
